...
define(['https://www.lactame.com/github/adobe-webplatform/Snap.svg/84fbff7d512c8145c522b71fc9c872cb0bcae49a/dist/snap.svg-min.js'], function (Snap) {
  ...
  var paper, svg, svgModifier; // Create svg

  function createSvg(electronicConfiguration, opts) {
    opts = Object.assign({}, defaultOptions, opts); // User defined
    ...
    var arrowHeadSize = opts.arrowHeadSize; // Calculated
    ...
    function drawElectron(layer, electron) {
      ...
      if (spin > 0) {
        p = "M".concat(spinOuterMargin, ",").concat(rectHeight - 1, " ").concat(spinOuterMargin, ",").concat(2 * arrowHeadSize + 1);
      } else {
        var margin = spinOuterMargin + spinInnerMargin;
        p = "M".concat(margin, ",2 ").concat(margin, ",").concat(rectHeight - (2 * arrowHeadSize + 1));
      }
    ...
    for (i = 0; i < 7; i++) {
      ...
      drawRect(leftRightMargin, i * w / 6 + topBottomMargin, "".concat(label, "1"));
  ...
  function getRectId(elConfig) {
    ...
    return "l".concat(q.l, "_m").concat(q.m, "_n").concat(q.n);
  }
  ...
  function getQuanticNumbers(elConfig) {
    ...
    switch (l) {
      ...
      default:
        throw new Error("Unexpected l: ".concat(l));
    }
